			...
			<view v-if="disdingwei" id="hot">
				...
				<view class="dingwei" v-if="Visit.length >= 0">
					...
					<view class="dingwei_city dingwei_city_zuijin">
						<view class="dingwei_city_one toright" v-for="(item, index) in Visit" v-if="index < 2" @click="back_city(item)">{{ item.cityName }}</view>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		back_city(item = this.city) {
			if (item) {
				this.$store.commit('LOC_CITY', item);
				...
				function distinct(arr) {
					...
					for (let i = 0; i < arr.length; i++) {
						if (!obj[arr[i].cityName]) {
							newArr.push(arr[i]);
							obj[arr[i].cityName] = true;
						}
					}
					return newArr;
				}
