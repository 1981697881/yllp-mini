<template>
  <view>
    <view style="position: relative" v-if="isHide">
      <view @tap="isHide = false" class="dt-content" :style="'-webkit-line-clamp:'+line">
        <text class="content">
          <slot>{{ dt ? dt : '' }}</slot>
        </text>
      </view>
      <view @tap="isHide = false" class="button-show"  v-if="enableButton&&lines>line">
        <text class="text-cyan">{{ expandText }}</text>
      </view>
    </view>
    <view v-else>
